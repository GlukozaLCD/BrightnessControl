...
public class ScheduleEngineTests
{
    ...
    // FP5 Фаза 3: профиль приложения приоритетнее расписания — пока
    // isMonitorSuppressed возвращает true для монитора, движок обязан оставить его
    // в покое (см. wiring в App.axaml.cs: предикат основан на AppProfileEngine).
    [Fact]
    public void EvaluateAndApply_SkipsSuppressedMonitor_ThenAppliesOnceUnsuppressed()
    {
        var statePath = Path.Combine(Path.GetTempPath(), $"schedule-state-test-{Guid.NewGuid():N}.json");
        var pacingPath = Path.Combine(Path.GetTempPath(), $"schedule-pacing-test-{Guid.NewGuid():N}.json");
        try
        {
            ...
            using var engine = new ScheduleEngine(controller, store, autoStart: false, isMonitorSuppressed: _ => true);
            ...
            try
            {
                ...
                engine.EvaluateAndApply(DateTime.Today.AddHours(9));
                var whileSuppressed = controller.GetBrightness(ddcCiMonitor);
                Assert.NotNull(whileSuppressed);
                Assert.InRange(whileSuppressed!.Percent, original - 5, original + 5);
            }
            finally
            {
                controller.SetBrightness(ddcCiMonitor, original);
